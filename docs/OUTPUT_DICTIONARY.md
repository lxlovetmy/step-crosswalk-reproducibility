...
Main Table 1 has 22 display rows (including three categorical section headings), Table 2 has seven algorithms, and Table 3 has nine illustrative directions. `publication_tables/supplemental_table_file_index.csv` maps the S1–S8 data layer; Table S2 is represented by the exact-knot file and the direction-metadata/index file.
