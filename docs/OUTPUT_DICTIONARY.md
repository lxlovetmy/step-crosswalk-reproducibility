...
| common-support files / Table S4 | eligible subgroup support bounds, full/common H, curve-estimator metadata | Eligible nonmissing subgroups require n>=200. Complete-sample H uses their full-data isotonic curves on the complete-sample source P05–P95 grid; common-support H uses those same curves on 101 equally spaced points in the intersected eligible-subgroup source P05–P95 interval. `common_support_linear_H` is the sole formal common-support field. |
...
| bootstrap files / Table S7 | point estimate, percentile CI, successful/requested repetitions | Stratified PSU-within-stratum cluster-bootstrap uncertainty; common-support output has 42 complete-sample and 42 single-grid common-support rows; no replicate rows are released |
| wear files / Table S8 | wear threshold, aggregate point estimates and change from 960 | Point-estimate sensitivity; common-support H delegates to the same full-data eligible-subgroup curves and 101-point equally spaced grid; bootstrap not recomputed |
